...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/OnUI-Pendro-1.5-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop org.onui-pendro.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop org.onui-pendro.device | grep -q "$CUSTOM_BUILD");
    then
...
function installrecovery()
{
    ...
    adb wait-for-online shell mount /system 2>&1 >> /dev/null
    adb wait-for-online remount
    if (adb shell getprop org.onui-pendroi.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.onui-pendro.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
